...
func TestProxyDial(t *testing.T) {
	...
	ws, _, err := cstDialer.Dial(s.URL, nil, nil)
	if err != nil {
		t.Fatalf("Dial: %v", err)
	}
	defer ws.Close()
...
func TestHandshakeTimeoutInContext(t *testing.T) {
	...
	defer cancel()
	ws, _, err := d.DialContext(ctx, s.URL, nil, nil)
	if err != nil {
		t.Fatal("Dial:", err)
	}
	ws.Close()
...
func TestDialBadScheme(t *testing.T) {
	...
	ws, _, err := cstDialer.Dial(s.Server.URL, nil, nil)
	if err == nil {
		ws.Close()
		t.Fatalf("Dial: nil")
	}
...
func TestDialBadOrigin(t *testing.T) {
	...
	ws, resp, err := cstDialer.Dial(s.URL, http.Header{"Origin": {"bad"}}, nil)
	if err == nil {
		ws.Close()
		t.Fatalf("Dial: nil")
	}
...
func TestDialBadHeader(t *testing.T) {
	...
	for _, k := range []string{"Upgrade",
		...
		h := http.Header{}
		h.Set(k, "bad")
		ws, _, err := cstDialer.Dial(s.URL, http.Header{"Origin": {"bad"}}, nil)
		if err == nil {
			ws.Close()
...
func TestDialExtraTokensInRespHeaders(t *testing.T) {
	...
	ws, _, err := cstDialer.Dial(makeWsProto(s.URL), nil, nil)
	if err != nil {
		t.Fatalf("Dial: %v", err)
	}
	defer ws.Close()
...
func TestHandshake(t *testing.T) {
	...
	ws, resp, err := cstDialer.Dial(s.URL, http.Header{"Origin": {s.URL}}, nil)
	if err != nil {
		t.Fatalf("Dial: %v", err)
	}
	defer ws.Close()
...
func TestRespOnBadHandshake(t *testing.T) {
	...
	ws, resp, err := cstDialer.Dial(makeWsProto(s.URL), nil, nil)
	if err == nil {
		ws.Close()
		t.Fatalf("Dial: nil")
	}
...
// TestHost tests handling of host names and confirms that it matches github.com/Noooste/fhttp.
func TestHost(t *testing.T) {
	...
	// Avoid log noise from github.com/Noooste/fhttp server by logging to testing.T
	...
	for i, tt := range tests {
		...
		c, resp, err := dialer.Dial(wsProtos[tt.server]+tt.url+"/", h, nil)
		if err == nil {
			c.Close()
		}
		...
		// Confirm that github.com/Noooste/fhttp has same result
...
func TestDialCompression(t *testing.T) {
	...
	ws, _, err := dialer.Dial(s.URL, nil, nil)
	if err != nil {
		t.Fatalf("Dial: %v", err)
	}
	defer ws.Close()
...
func TestNetDialConnect(t *testing.T) {
	...
	for _, tc := range tests {
		...
		// Test websocket dial
		c, _, err := dialer.Dial(testUrls[tc.server], nil, nil)
		if err != nil {
...
func TestNextProtos(t *testing.T) {
	...
	// after the Client.Get call from github.com/Noooste/fhttp above.
	...
	_, _, err = d.Dial(makeWsProto(ts.URL), nil, nil)
	if err == nil {
